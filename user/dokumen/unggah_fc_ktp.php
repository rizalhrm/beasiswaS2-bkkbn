...
?>
  ...
  <legend style="margin-bottom: 10px; font-size: 15px; font-weight: bold">Unggah KTP</legend>
  <div class="row-fluid">
    <?php
      ...
      Anda harus mengisi formulir terlebih dahulu
       ...
       onsubmit="if (eval(ukurandok )>1.5) { alert('Ukuran File ' + ukurandok + ' MB Melebihi Batas yaitu 1.5 MB.'); return false; } else { return pdf(); }">
         ...
         <div class="control-group" align="center">
           ...
           <input type="file" id="file" name="fc_ktp" value="" accept=".pdf" required>
         </div>
         <div class="control-group" align="center">
  ...
  <?php
   if (isset($_POST['simpan'])) {
     ...
     $lokasi= "../file/dokumen/"."Kartu_Tanda_Penduduk_".$nip."_".basename($fc_ktp);
     $name_file = "Kartu_Tanda_Penduduk_".$nip."_".basename($fc_ktp);
     if($name_file){
     copy($_FILES['fc_ktp']['tmp_name'],$lokasi);
     }
     $queryfc_ktp="UPDATE dokumen SET fc_ktp='$name_file' WHERE nip='$nip'";
     $simpanfc_ktp=mysqli_query($koneksi,$queryfc_ktp);
     if ($simpanfc_ktp) {
